...
recommend: false
toc: ture
image: "https://i.p-i.vip/89/20251114-6916d6c9c02da.webp"
---
...
别被这个名字吓到，「无头」意思就是它只提供一个内容管理的后台，但不管你的网站「前台」长啥样。这正好跟 Hugo 这种静态网站生成器是天生一对！

装上 TinaCMS 之后，它能帮你在你自己的 Hugo 网站上变出一个管理后台（就像 `http://你的网域/admin 这样`）。在这个后台裡，你可以：
...
如果你暂时还不想折腾「线上编辑」，只是想在自己电脑上有个更舒服的写作后台（不想天天对著 Markdown 原始码），那可以先试试它的「本地模式」。这个模式下，所有的修改都会直接保存在你电脑上的 Git 仓库裡。
...
安装过程中，它会问你一些问题，记得在「框架」那裡选择 `Hugo`。
...
4. **配置 TinaCMS：** 这一步最关键。找到 `tina/config.ts` 这个文件，它会告诉 TinaCMS 你的文章都放在哪个文件夹裡。 比如，我的文章放在 `content/posts` 文件夹裡，那我就要改成这样：
...
注意： 这裡的 path 一定要改成你自己放文章的目录！`fields` 裡的 `name` 和 `label` 是你在后台看到的字段名称，`isTitle` 和 `isBody` 是告诉 TinaCMS 这个字段是标题和正文。
...
提示： 这些是敏感信息，最好通过 `Vercel/Netlify` 的「环境变量」来设置，而不是直接写死在代码裡再推送到 `GitHub` 上。

4. **修改「网站部署」指令：** 这一步很重要！以前我们部署 Hugo 网站（比如在 Vercel 或 Netlify 上），部署指令可能就是简单的 hugo。 现在，因为我们加入了 TinaCMS，所以部署指令要改成 `npm run build` (就是我们在 `package.json` 裡定义的那个 `tinacms build && hugo`)。 这样，部署平台在打包你的网站时，才会先把 TinaCMS 的后台管理界面也一起打包进去。
...
如果你也受够了繁琐的本地编辑流程，或者想让不懂技术的运营伙伴也能帮你更新网站内容，那麽 TinaCMS 绝对值得你花点时间折腾一下！