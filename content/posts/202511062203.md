...
recommend: true
toc: false
image: "https://s0.wp.com/mshots/v1/https://frontmatter.codes/"
---
...
![](https://i.p-i.vip/89/20251114-6916d52d99dd4.webp)
...
安装 Hugo 和 Git，并确保你的 Hugo 网站项目已建立。
...
Front Matter 允许你把 Shortcode 变成图形化菜单，以后就可以通过点击按钮来插入 Shortcode。
...
打开 VS Code，打开 Hugo 根目录文件夹。

切换到 Front Matter (FM) 页面，点击 Create Content (创建内容)。

选择要发布的 section (例如 posts)，输入文章标题。

Front Matter 会自动创建好 Page Bundle 目录和 index.md 文件。
...
设置缩略图： Front Matter 列表的缩略图需要手动指定才会显示。我仿造 Blowfish 主题的习惯，将 Page Bundle 目录下、与 index.md 同级的 feature.webp 文件设置为文章缩略图 (在 front matter 中指定 preview 字段)。
